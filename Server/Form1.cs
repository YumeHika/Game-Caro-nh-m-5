...
namespace Server
{
    public partial class Form1 : Form
    {
        ...
        private void danhsachnguoichoi(Player ple)
        {
            ...
            string danhsachnguoichoi = "DANHSACHNGUOICHOI|,";
            danhsachnguoichoi += string.Join(",", player.Select(p => p.name).ToArray());
            data = Encoding.Unicode.GetBytes(danhsachnguoichoi);
            ple.socket.Send(data, data.Length, SocketFlags.None);
        }
